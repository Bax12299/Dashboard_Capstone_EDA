...
.nav-label { font-size: 0.73rem; font-weight: 600; color: #475569; padding-right: 0.4rem; white-space: nowrap; }
.nav-btn {
...
.nav-btn:hover { background: var(--bg-3); border-color: var(--border); color: var(--text); }
.bq-num {
...
def base_layout(title="", margin=None):
    return dict(
        title=dict(
            text=title,
            font=dict(size=13, color=TEXT_COLOR, family="DM Sans"),
...
bar_colors_bq1 = [PURPLE if i < 5 else "#6D28D9" for i in range(len(top_skills_df))]
...
<div class="section-header">
    ...
    <div>
        ...
        <p>Objektif : memetakan distribusi Remote, Hybrid, dan On-site untuk setiap kategori pekerjaan IT.</p>
    </div>
</div>
""", unsafe_allow_html=True)
...
colors_wt   = {"Remote": PURPLE, "Hybrid": PURPLE_MID, "On-site": "#4C1D95"}
...
with col_r1:
    ...
    for kw, pct in top5_remote.items():
        st.markdown(f"""
        <div class="rank-item" style="background:#2D1B69;border-color:#3B1FA8;">
            <span style="font-size:0.82rem;font-weight:600;color:#F1F5F9;">{kw}</span>
...
with col_r2:
    ...
    for kw, pct in top5_onsite.items():
        st.markdown(f"""
        <div class="rank-item" style="background:#1E293B;border-color:#334155;">
            <span style="font-size:0.82rem;font-weight:600;color:#F1F5F9;">{kw}</span>
            <span style="font-size:0.82rem;font-weight:700;color:#94A3B8;">{pct:.0f}%</span>
        </div>
        """, unsafe_allow_html=True)
...
n_c          = len(top_complex)
bar_colors_c = [PURPLE if i < 5 else "#6D28D9" if i < 10 else PURPLE_LITE for i in range(n_c)]
...
with col_bq3b:
    ...
    for _, row in top_complex.head(5).iterrows():
        st.markdown(f"""
        <div class="rank-item" style="background:#2D1B69;border-color:#3B1FA8;">
            <span style="font-size:0.79rem;font-weight:600;color:#F1F5F9;">{row['keyword']}</span>
...
if heat_rows and all_d_skills:
    ...
    fig_bq4 = go.Figure(go.Heatmap(
        ...
        colorscale=[[0, "#1E293B"], [0.3, "#2D1B69"], [0.7, "#6D28D9"], [1, PURPLE]],
        ...
        showscale=True,
        colorbar=dict(title="Ratio", tickfont=dict(size=10, color=TEXT_COLOR),
                      title_font=dict(color=TEXT_COLOR)),
    ))
    fig_bq4.update_layout(
        **base_layout("Skill Khas per Kategori Pekerjaan",
                      margin=dict(l=0, r=60, t=42, b=80)),
        height=400,
    )
    apply_base_axes(fig_bq4)
    fig_bq4.update_xaxes(tickangle=-35, tickfont=dict(size=10, color=TEXT_COLOR))
    fig_bq4.update_yaxes(tickfont=dict(size=10, color=TEXT_COLOR))
...
<div class="insight-box">
    ...
    saja. Nilai tinggi berarti skill itu sangat identik dengan pekerjaan tersebut, sebagai contoh
    <strong>Dart dan Flutter</strong> hampir hanya muncul di lowongan Flutter Developer,
...
<div class="section-header">
    ...
    <div>
        ...
        <p>Objektif : Membandingkan jumlah skill yang diminta berdasarkan tipe kerja untuk melihat apakah ada "skill premium" pada posisi remote.</p>
    </div>
</div>
""", unsafe_allow_html=True)
...
with col_bq5a:
    fig_bq5a = go.Figure(go.Bar(
        ...
        y=wt_stats["mean"],
        marker_color=[PURPLE, PURPLE_MID, "#4C1D95"],
...
with col_bq5b:
    fig_bq5b = go.Figure()
    colors_v = {"Remote": PURPLE, "Hybrid": PURPLE_MID, "On-site": "#4C1D95"}
    for wt in wt_order:
...
if remote_distinct:
    ...
    for i, (sk, r_pct, o_pct) in enumerate(remote_distinct):
        ...
        <div style="background:#1E293B;border:1px solid #334155;border-radius:9px;
            ...
            <div style="font-size:0.73rem;font-weight:700;color:#F1F5F9;margin-bottom:0.3rem">{sk}</div>
            <div style="font-size:1rem;font-weight:700;color:{PURPLE};">{r_pct:.0f}%</div>
            <div style="font-size:0.67rem;color:#64748B;">remote</div>
            <div style="font-size:0.68rem;color:#34D399;margin-top:0.18rem;">+{diff:.0f}% vs on-site</div>
        </div>
        """, unsafe_allow_html=True)
...
with col_bq6a:
    ...
    bc6 = [PURPLE if i < 5 else "#4C1D95" for i in range(len(pairs_df))]
...
with col_bq6b:
    ...
    fig_bq6b = go.Figure(go.Heatmap(
        ...
        colorscale=[[0, "#1E293B"], [0.4, "#2D1B69"], [1, PURPLE]],
    ...
    fig_bq6b.update_xaxes(tickangle=-35, tickfont=dict(size=9, color=TEXT_COLOR), side="bottom")
    fig_bq6b.update_yaxes(tickfont=dict(size=9, color=TEXT_COLOR), autorange="reversed")
...
<div style="text-align:center;padding:0.875rem 0 1.75rem;color:#475569;
